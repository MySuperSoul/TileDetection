...
class Mosaic(object):
    ...
    def __init__(self, p=0.3, train_pickle_path=None, train_imgs_folder=None):
        self.p = p
        ...
        for anno in annos:
            self.name_anno_map[anno['filename']] = anno

        self.train_filenames = list(self.name_anno_map.keys())
    ...
    def __call__(self, results):
        ...
        if random.random() < self.p:
            ...
            selected_names = rd.sample(self.train_filenames, 3)
    ...
    def __call__(self, results):
        ...
        Returns:
            dict: Normalized results, 'img_norm_cfg' key is added into
                result dict.
        """
        if 'concat' in results.keys() and results['concat'] == True:
            img_raw, img_temp = results['img'][:, :, :3], results['img'][:, :,
                                                                         3:]
...
class Albu(object):
    ...
    def __call__(self, results):
        ...
        if results['gt_labels'].size == 0 or results['gt_bboxes'].size == 0:
            return results
