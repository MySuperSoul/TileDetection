...
class CascadeRoIHead(BaseRoIHead, BBoxTestMixin, MaskTestMixin):
    ...
    def _bbox_forward_train(self, stage, x, sampling_results, gt_bboxes,
        ...
        # Apply ISR-P
        isr_cfg = rcnn_train_cfg.get('isr', None)
        if isr_cfg is not None:
            bbox_targets = isr_p(
                ...
                rois,
                sampling_results,
                self.bbox_head[stage].loss_cls,
                self.bbox_head[stage].bbox_coder,
                **isr_cfg,
                num_class=self.bbox_head[stage].num_classes)
        loss_bbox = self.bbox_head[stage].loss(cls_score, bbox_pred, rois,
                                               *bbox_targets)

        # Add CARL Loss
        carl_cfg = rcnn_train_cfg.get('carl', None)
        if carl_cfg is not None:
            loss_carl = carl_loss(
                ...
                bbox_pred,
                bbox_targets[2],
                self.bbox_head[stage].loss_bbox,
                **carl_cfg,
                num_class=self.bbox_head[stage].num_classes)
