...
from mmcv.cnn import constant_init, kaiming_init, build_norm_layer, ConvModule
    ...
    def __init__(self,
        ...
        if separable_conv:
            ...
            self.conv_op = nn.Sequential(
                SeparableConv(
                    in_channels, out_channels, bias=True, relu=False),
                bn_layer)
        else:
...
class bifpn(nn.Module):
    ...
    def forward(self, inputs):
        ...
        for fnode, op_node, new_op_node in zip(self.nodes_settings,
                                               self.resample_op_nodes,
                                               self.new_op_nodes):
            input_node = []
            for input_offset, resample_op in zip(fnode['inputs_offsets'],
                                                 op_node):
...
class BiFPN(nn.Module):
    ...
    def __init__(self,
        ...
        bifpn_in_channels = in_channels[self.start_level:self.
                                        backbone_end_level]
